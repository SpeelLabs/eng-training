...
**フォーム (form)** は、ユーザーが情報を入力してサーバーに送信するための仕組みです。お問い合わせ、ログイン、アンケート、検索など、Web サイトの対話機能の多くはフォームを使用しています。
...
* `action`: フォームデータの送信先 URL
* `method`: データの送信方法 ( `get` または `post` )

> 📝 **ポイント**: はじめは `action` や `method` の詳細を理解する必要はありません。フォームのレイアウトと各要素の使い方を中心に学びましょう。
...
HTML のバリデーション機能を使って、入力値の検証ができます。
...
> 📝 **ポイント**: HTML のバリデーション機能は手軽ですが、本格的なアプリケーションでは JavaScript を使ったより詳細な検証も行います。
